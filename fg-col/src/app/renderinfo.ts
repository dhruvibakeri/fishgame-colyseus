...
export function genFishSpec(totalFishes: number, x: number, y: number, hexSize: number,
...
export function stateToPenguinDisplayInfo(state: State): { color: string, place: [number, number] }[] {
...
// "red" -> "fill: red; "
export const colorToSVGFillStyle = (color: string) =>
  `fill:${color} ;`

// converts the nose corners to an svg-compatible string
export const triangleCornersToString = ({ l, r, b }: {
  l: { x: number, y: number },
  r: { x: number, y: number },
  b: { x: number, y: number },
