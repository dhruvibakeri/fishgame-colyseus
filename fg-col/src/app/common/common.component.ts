...
export const substates = (tree: Tree): State[] =>
  applyDirectlyReachable(tree, identity);
export const stateFromTree = (tree: Tree): State =>
  isArray(tree) ? tree[0] : tree;

/** Apply a function f to all states directly reachable from tree. */
export const applyDirectlyReachable = <T>(
  tree: Tree,
  f: (t: State) => T
): T[] => (isArray(tree) ? map(f, map(stateFromTree, subtrees(tree))) : []);

/** Creates a game tree from a given base state using "next" transition function. */
export const fishTreeFromState = (state: State): Tree => [
  state,
  () => next(state),
];

/** ASSUMPTION: initial board will be big enough to place all the penguins for 2, 3, 4 players. */
export const next = (state: State): Tree[] =>
  state.stage === 'placing' ? nextPlacing(state) : nextMoving(state);

/** "next" when the stage is "placing" */
export const nextPlacing = (state: State): Tree[] =>
  (isLastPlacement(state)
    ? transitionStage(placeStates(state))
    : placeStates(state)
  ).map(fishTreeFromState);

/** States with each possible penguin placed. */
export const placeStates = (state: State): State[] =>
  map((p) => placeAvatar(p, state), allPlaceablePositions(state));

/** Is only one penguin left to be placed? */
export const isLastPlacement = (state: State): boolean =>
  toPlace(state.players.length) - getPenguinPosns(state).length === 1;

/** "next" when the stage is "playing" */
export const nextMoving = (state: State): Tree[] =>
  isGameOver(state) ? [] : moveGameNotOver(state);

/** ASSUMPTION: game not over in state */
export const moveGameNotOver = (state: State) =>
  isEmpty(moveStates(state))
    ? [fishTreeFromState(skipMove(state))]
    : moveStates(state).map(fishTreeFromState);

/** A "skip move" transition on state. */
export const skipMove = (state: State): State => ({
  stage: state.stage,
  board: state.board,
  players: append(head(state.players), tail(state.players)),
});
...
export const transitionStage = (states: State[]): State[] =>
  map(set(lensProp('stage'), 'playing'), states);
...
export const moveStates = (state: State): State[] =>
  unnest(
    map(
      (place) =>
        map(
          (movePos) => moveAvatar(place, movePos, state),
          validMovePosns(state, place)
        ),
      state.players[0].places
    )
  );
...
export const allPlaceablePositions = (state: State): Posn[] =>
  filter(
    (posn) => !isUnreachablePosn(state, posn),
    unnest(
      map(
        (e) =>
          <Posn[]>(
            zip(
              repeat(e, state.board[0].length),
              times(identity, state.board[0].length)
            )
          ),
...
export const boardDimToCorners = (
  size: number,
  rin: number,
  cin: number,
  offset: number
): [Posn, Posn, Posn, Posn, Posn, Posn][][] =>
  boardMap(sameFishBoard(0, rin, cin), (_r, rid, _c, cid) =>
    hexCoordToCorners(size, rid, cid, offset)
  );

/** MATH. Maps [row, col] to its associated hex corners [xPx, yPx][]  */
export const hexCoordToCorners = (
  size: number,
  rowIndex: number,
  colIndex: number,
  offset: number
): [Posn, Posn, Posn, Posn, Posn, Posn] =>
  cornersFromWest(size, [
    rowIndex % 2 === 0
      ? 4 * colIndex * size + offset
      : (colIndex * 4 + 2) * size + offset,
    (rowIndex + 1) * size + offset,
  ]);

/** Generate all other corners (going clockwise from west) from the west corner. */
export const cornersFromWest = (
  size: number,
  [xPxWest, yPxWest]: Posn
): [Posn, Posn, Posn, Posn, Posn, Posn] => [
  [yPxWest, xPxWest],
  [yPxWest - size, xPxWest + size],
  [yPxWest - size, xPxWest + 2 * size],
  [yPxWest, xPxWest + 3 * size],
  [yPxWest + size, xPxWest + 2 * size],
  [yPxWest + size, xPxWest + size],
];

/** Gives the size of the hex board. Width: all NW+N edges + size, height: all NW edges + size. */
export const specToBackgDim = (
  size: number,
  r: number,
  c: number
): [number, number] => [c * 2 * 2 * size + size, r * size + size];

/** create an initial game state with r by c board and n players. Flag for assigning rand cols to players. */
export const playersToGameState = (
  r: number,
  c: number,
  n: number,
  randomize: boolean
  ...
  stage: 'placing',
  board: oneFishSpecificHoles(0, [], r, c),
  players: initPlayers(n, randomize),
});
...
export const initPlayers = (n: number, randomize: boolean): Player[] =>
  map(
    (c) => ({ score: 0, status: 'online', color: c, places: [] }),
    genColors(n, randomize)
  );

/** (Optionally shuffled) array of n colors. ASSUMPTION: n should be in [2, 4].*/
export const genColors = (n: number, randomize: boolean) =>
  take(n, randomize ? shuffle(COLORS) : COLORS);

/** Put holes in holePosns and 1 fishes otherwise. */
export const oneFishSpecificHoles = (
  minOneFishes: number,
  hPosns: Posn[],
  r: number,
  c: number
): Board => nFishSpecificHoles({ fishes: 1, min: minOneFishes }, hPosns, r, c);

/** Put holes in their positions otherwise N fishes */
export const nFishSpecificHoles = (
  minNFish: { fishes: number; min: number },
  holePosns: Board,
  rows: number,
  cols: number
): Board =>
  boardMap(sameFishBoard(0, rows, cols), (_r, rid, _c, cid) =>
    any(equals([cid, rid]), holePosns) ? -1 : minNFish.fishes
  );

/** At least min fishes and the rest of the positions are randomized (0 to 5) */
export const nonHoleTiles = (
  minNFish: { fishes: number; min: number },
  total: number
): number[] =>
  shuffle(
    concat(
      times((_) => random(0, 5), total - minNFish.min),
      repeat(minNFish.fishes, minNFish.min)
    )
  );

/** f # of fish on every tile, no holes, r by c board. */
export const sameFishBoard = (f: number, r: number, c: number): Board =>
  repeat(repeat(f, c), r);

export const removeTileAt = (b: Board, row: number, col: number): Board =>
  boardMap(sameFishBoard(0, b.length, b[0].length), (_r, rid, _c, cid) =>
    equals([cid, rid], [row, col]) ? -1 : b[rid][cid]
  );

export const boardMap = <T, D>(
  b: T[][],
  f: (r: T[], rid: number, c: T, cid: number) => D
): D[][] =>
  addIndex(map)(
    (r: T[], rid) => addIndex(map)((c: T, cid: number) => f(r, rid, c, cid), r),
    b
  );
...
export const xtree = (input: {
  state: InputState;
  from: Posn;
  to: Posn;
}): Move | false =>
  getBestMovesAndTieBreak(
    input.to,
    possibleNeighMoves(input.from, input.to, inpSt2St(input.state))
  );

export const xstrategy = ([depth, inputState]: [number, InputState]):
  | false
  | Move => minimaxAction(depth)(inpSt2St(inputState));

export const xref = (gameDescription: GameDescription): string[] =>
  map(
    (player) => player.name,
    getWinners(playGame(descriptionToState(gameDescription, false)))
  ).sort();

/** If maybeState is false, return it, otherwise convert it to input state and return */
export const xStateRes = (maybeState: State | false): InputState | false =>
  maybeState && st2InpSt(maybeState);
...
export const maybeStateFromAction = (
  ...
  isPosn(action)
    ...
    : maybeInSubstates(
        tree,
        moveAvatar(head(action), last(action), stateFromTree(tree))
      );
...
export const isPosn = (moveOrPosition: Posn | Move): moveOrPosition is Posn =>
  isArray(moveOrPosition) &&
  moveOrPosition.length === 2 &&
  isNumber(moveOrPosition[0]) &&
  isNumber(moveOrPosition[1]);

/** Simulates a game using the given state */
export const playGame = (state: State): State =>
  isGameOver(state)
    ? state
    : gamePlayMakeMove(state, minimaxAction(currentPlayer(state).depth)(state));
...
export const gamePlayMakeMove = (state: State, maybeMove: false | Move) =>
  maybeMove === false
    ? playGame(skipMove(state))
    : playGame(moveAvatar(from(maybeMove), to(maybeMove), state));
...
export const getBestMovesAndTieBreak = (to: Posn, possibleMoves: Move[]) =>
  not(isEmpty(possibleMoves)) &&
  tieBreaker(getBestNeighborMoves(possibleMoves, neighbors(to)));

/** select moves for each neighbor (ordered clockwise starting North), find the first non-empty moves arr, otherwise return [] */
export const getBestNeighborMoves = (
  moves: Move[],
  neighbors: Posn[]
): Move[] =>
  defaultTo([])(
    find(
      (l) => not(isEmpty(l)),
      map((dirPosn) => inSelect(moves, to, [dirPosn]), neighbors)
    )
  );

/** All xs such that for each x, xToY(x) exists in ys */
export const inSelect = <Y, X>(xs: X[], map: (x: X) => Y, ys: Y[]): X[] =>
  filter((x) => includes(map(x), ys), xs);

/** All possible moves to the neighbors of "toP" */
export const possibleNeighMoves = (
  fromP: Posn,
  toP: Posn,
  state: State
): Move[] =>
  inSelect(
    subseqNoSkipMoves(moveAvatar(fromP, toP, state)),
    to,
    neighbors(toP)
  );
...
export const subseqNoSkipMoves = (state: State): Move[] =>
  map(
    (substate) => fromStateToStateMove(state, substate),
    subseqNoSkipStates(state)
  );

/** Filter all possible substates that are generated by a "skip" move */
export const subseqNoSkipStates = (state: State): State[] =>
  filter(
    (toState) => notSkipMove(state, toState),
    substates(fishTreeFromState(state))
  );
...
export const noSkipMove = (move: Move | false): false | Move =>
  move && !equals(from(move), to(move)) && move;
...
export const placeAvatar = (p: Posn, s: State): State =>
  assoc(
    ...
    append(
      assoc(
        'places',
        append(p, head(getPlayers(s)).places),
        head(getPlayers(s))
      ),
      tail(getPlayers(s))
    ),
...
export const mkHole = (p: Posn, b: Board): Board =>
  boardMap(b, (_r, rid, c, cid) => (equals([rid, cid], p) ? -1 : c));

/** Moves the avatar within the players field of a state from "from" to "to" */
export const moveAvatarInPlayers = (
  from: Posn,
  to: Posn,
  players: Player[],
  fishesToCollect: number
): Player[] =>
  append(
    assoc(
      'score',
      head(players).score + fishesToCollect,
      assoc(
        'places',
        replacePosn(head(players).places, from, to),
        head(players)
      )
    ),
    tail(players)
...
export const replacePosn = (posns: Posn[], from: Posn, to: Posn): Posn[] =>
  update(findIndex(equals(from), posns), to, posns);
...
export const fromStateToStateMove = (fromState: State, toState: State): Move =>
  fromPlacesToPlacesMove(
    getPlaces(currentPlayer(fromState)),
    getPlaces(lastPlayer(toState))
  );
...
const fromPlacesToPlacesMove = (froms: Posn[], tos: Posn[]): Move =>
  equals(froms, tos)
    ? [head(froms), head(froms)]
    : [head(without(tos, froms)), head(without(froms, tos))];

/** Can any player move their avatar? */
export const canAnyPlayerMoveAvatar = (state: State): boolean =>
  any(
    (place) => movesLeft(state, place),
    unnest(state.players.map((p) => p.places))
  );

export const isGameOver = (state: State): boolean =>
  !canAnyPlayerMoveAvatar(state);

/** Are there valid move position from p in s? */
export const movesLeft = (s: State, p: Posn): boolean =>
  validMovePosns(s, p).length > 0;
...
export const getWinners = (state: State): Player[] =>
  maxBys(
    ...
    (player) =>
      player.score +
      sum(player.places.map((place) => state.board[place[0]][place[1]]))
  );

/** ------------------------------------------------------ reachability ------------------------------------------------------ */

export const straightListPosns = (s: State, p: Posn) =>
  posnReachable(s, p, isPosnOutOfBounds);
export const validMovePosns = (s: State, p: Posn) =>
  posnReachable(s, p, isUnreachablePosn);
export const isUnreachablePosn = (s: State, p: Posn): boolean =>
  isPosnOutOfBounds(s, p) || isHoleAtPos(s, p) || isPenguinAtPosn(s, p);
export const isPenguinAtPosn = (s: State, p: Posn): boolean =>
  any(equals(p), getPenguinPosns(s));
export const isPosnOutOfBounds = (s: State, p: Posn): boolean =>
  !isPositionInBoard(s, p);
...
export const posnReachable = (
  s: State,
  p: Posn,
  unreachable: (s: State, p: Posn) => boolean
): Posn[] =>
  unnest([
    inDir(s, p, north, unreachable),
    inDir(s, p, northEast, unreachable),
    inDir(s, p, southEast, unreachable),
    inDir(s, p, south, unreachable),
    inDir(s, p, southWest, unreachable),
    inDir(s, p, northWest, unreachable),
  ]);
...
export const inDir = (
  ...
  move: (p: Posn) => Posn,
  unreachable: (s: State, p: Posn) => boolean
): Posn[] => reachablePathInDirAcc(s, move, unreachable, [], move(p));
...
export const northEast = ([r, c]: Posn): Posn =>
  r % 2 === 0 ? [r - 1, c] : [r - 1, c + 1];
export const southEast = ([r, c]: Posn): Posn =>
  r % 2 === 0 ? [r + 1, c] : [r + 1, c + 1];
export const south = ([r, c]: Posn): Posn => [r + 2, c];
export const southWest = ([r, c]: Posn): Posn =>
  r % 2 === 0 ? [r + 1, c - 1] : [r + 1, c];
export const northWest = ([r, c]: Posn): Posn =>
  r % 2 === 0 ? [r - 1, c - 1] : [r - 1, c];
...
export const isHoleAtPos = (s: State, p: Posn): boolean =>
  s.board[p[0]][p[1]] === -1;
export const getPenguinPosns = (s: State): Posn[] =>
  flatten(map(getPlaces, getPlayers(s)));
...
export const minimaxAction = (depth: number) => (state: State): Move | false =>
  noSkipMove(
    minimaxActionTree(state, subtrees(fishTreeFromState(state)), depth)
  );

/** Applies minimax to get a move on given state (and its subtrees), false if no move can be made */
export const minimaxActionTree = (
  state: State,
  subtrees: Tree[],
  depth: number
): Move | false =>
  !isEmpty(subtrees) &&
  tieBreaker(
    maxBys(movesAndScores(state, subtrees, depth), getScore).map(getMove)
  );
...
export const movesAndScores = (
  ...
  map(
    (t) => [
      fromStateToStateMove(s, stateFromTree(t)),
      minimaxScore(t, currentPlayer(s).color, depth),
    ],
    subtrees
  );

/** Applies minimax score on the current state of the tree for the given depth */
export const minimaxScore = (
  tree: Tree,
...
export const minimaxScoreState = (
  s: State,
  tree: Tree,
  main: string,
  depth: number
): number =>
  depth === 1 || isGameOver(s)
    ? plrScore(s, main)
    : minimaxScoreStateNotOver(
        tree,
        main,
        depth,
        currentPlayer(s).color === main
      );

/** gets the score for the given color */
export const plrScore = (s: State, col: string): number =>
  filter((plr) => plr.color === col, s.players)[0].score;

/** Gets the minimal or maximal score depending on whether the player is the main player or not */
export const minimaxScoreStateNotOver = (
  tree: Tree,
  plr: string,
  depth: number,
  isplr: boolean
  ...
  isplr
    ? apply(Math.max, minmaxscore(tree, plr, depth, isplr))
    : apply(Math.min, minmaxscore(tree, plr, depth, isplr));
...
export const minmaxscore = (
  ...
  applyDirectlyReachable(tree, (s) =>
    minimaxScore(fishTreeFromState(s), plr, isplr ? depth - 1 : depth)
  );
...
export const zigZagAcc = (
  ...
  placed === toPlace || upto === placed
    ...
    : zigZagAcc(
        toPlace,
        placed + 1,
        placeAvatar([Math.floor(placed / colLen), placed % colLen], state),
        colLen,
        upto
      );
...
export const zeroesToHoles = (inpB: InputBoard): Board =>
  map((row) => map((col) => (0 === col ? -1 : col), row), inpB);
...
export const addHolesInputBoard = (inpB: InputBoard): InputBoard =>
  map(
    (r: number[]) =>
      r.length < maxRowLen(inpB)
        ? concat(r, repeat(0, maxRowLen(inpB) - r.length))
        : r,
    inpB
  );

/** The # of elements the longest row has */
export const maxRowLen = (inputBoard: number[][]): number =>
  apply(Math.max, map(length, inputBoard));

/** Convert an input board to board by adding missing holes and converting zeroes to holes */
export const inputBoardToBoard = (board: InputBoard): Board =>
  zeroesToHoles(addHolesInputBoard(board));

/** convert input board to state by adding stage field, an empty players field, and converting the board. */
export const inputBoardToState = (board: InputBoard): State => ({
  stage: 'placing',
  board: inputBoardToBoard(board),
  players: [],
});
...
export const inpSt2St = (inputState: InputState): State => ({
  stage: 'playing',
  board: inputBoardToBoard(inputState.board),
  players: inputPlayersToPlayers(inputState.players),
});

/** input players are converted to players by adding a status key */
export const inputPlayersToPlayers = (plrs: InputPlayer[]): Player[] =>
  <Player[]>map(assoc('status', 'online'), plrs);

/** board are converted to input board by converting -1 holes to zeroes */
export const boardToInputBoard = (board: Board): InputBoard =>
  map((row) => map((col) => (col === -1 ? 0 : col), row), board);
...
export const st2InpSt = (s: State): InputState => ({
  board: boardToInputBoard(s.board),
  players: <InputPlayer[]>map(dissoc('status'), s.players),
});

/** Creates initial players from an array of colors */
export const colsToPlayers = (
  cs: string[],
  ns: string[],
  ds: number[]
): Player[] =>
...
export const descriptionToState = (
  gd: GameDescription,
  shufle: boolean
): State =>
  gdInfoToState(
    gdToBoard(gd),
    map((pair) => pair[0], gd.players),
    map((pair) => pair[1], gd.players),
    genColors(gd.players.length, shufle)
  );

/** Runs a zig-zag placement on board using colors, and creates a player info from names, colors, and depths. */
export const gdInfoToState = (
  board: Board,
  names: string[],
  depths: number[],
  colors: string[]
  ...
  zigZagPlacement({
    stage: 'playing',
    board: board,
    players: colsToPlayers(colors, names, depths),
  });
...
export const gdToBoard = (gd: GameDescription): Board =>
  nFishSpecificHoles(
    { fishes: gd.fish, min: gd.row * gd.column },
    [],
    gd.row,
    gd.column
  );
...
export const names = (n: number) =>
  take(
    n,
    map((n) => String.fromCharCode(97 + n), range(0, n))
  );

/** makes a game description from the names */
export const gameDesc = (names: string[]): GameDescription => ({
  row: 5,
  column: 5,
  players: map((n) => [n, 2], names),
  fish: 2,
});
...
export const chunk = (names: string[]): string[][] =>
  names.length === 3 || names.length === 2
    ...
    : names.length % 4 === 1
    ? concat(
        splitEvery(4, dropLast(5, names)),
        splitEvery(3, takeLast(5, names))
      )
    : splitEvery(4, names);
...
export const runRound = (players: string[]): string[] =>
  reduce(
    <(l: string[], r: string[]) => string[]>concat,
    [],
    map(xref, map(gameDesc, chunk(players)))
  );
...
export const runTournament = (players: string[]): string[] =>
  players.length < 2
    ...
    ? xref(gameDesc(players))
    : dispatchFirstRound(runRound(players));
...
export const dispatchFirstRound = (players: string[]): string[] =>
  players.length < 2
    ...
    : players.length >= 2 && players.length <= 4
    ? xref(gameDesc(players))
    : dispatchSecondRound(players, runRound(players));

/** run the second round of, and recur on running the tournament if result is not the same as the first */
export const dispatchSecondRound = (
  firstRunRes: string[],
  secondRunRes: string[]
): string[] =>
  equals(secondRunRes, firstRunRes)
    ? secondRunRes
    : runTournament(secondRunRes);
