...
describe("make a move", () => {
   ...
   const board4 = [[ {kind: "usableSpace", occupiedBy: {tileInfo: {size: 75, maxElements: 1}, occupiedBy: {kind: "penguin", color: "red"}}},
    ...
    it("player1 makes move again", () => {
        assert.deepEqual(makeMove(1234, { row: 1, col: 0 }, { row: 0, col: 0 }, move3456), {gameStage: "playing", board: board4, nextMove: false, players: players})
    })
...
describe("checks if game is still on / not over", () => {
    ...
    it("after 1st player played turn again", () => {
        assert.deepEqual( isGameOn(move3456), false)
    })
    it("game is over", () => {
        assert.deepEqual( isGameOn(move1234again), false)
    })
