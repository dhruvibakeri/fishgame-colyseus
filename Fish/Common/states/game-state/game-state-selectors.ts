...
export function GET_GameStateKind(gs : GameState) : string {
 ...
 export function GET_GameStateNextToPlace(gs : GameState) : Player {
    if(GET_GameStateKind(gs) != "joining" && GET_GameStateKind(gs) != "done" ) {
        return (gs as GameStatePlacing | GameStatePlaying).nextToPlace
    }
}
